...
**Implementation notes** (executed in one continuous pass, as specs 001/003/004 were):

- **T004/build-insights.ts was written directly with real logic**, not literal per-story placeholders — all six card-type modules (T010, T014, T018, T022, T026, T031/T032) were built before wiring, then `build-insights.ts` was written once against the real functions. `missingDataExplanation` is populated whenever a card type's `cards.length === 0`, for every one of the six types uniformly (including plateau) — FR-015 requires a card-type-level explanation whenever nothing qualifies, and this session's own earlier T019 draft wording ("no explanation owed for this specific exercise") was about not explaining *each individual non-qualifying exercise*, not about skipping the plateau section's own explanation entirely; implemented per the literal FR-015 requirement.
- **`src/application/progression/most-recent-load.ts`** (not in the original task list): `progression-screen.tsx`'s (spec 004) private `mostRecentNumericLoad` helper was extracted into the application layer so `recent-records.ts` (T014) could reuse the same "reps at a fixed load" default without `application/insights/` reaching into `presentation/` (a layering violation) or duplicating the logic. `progression-screen.tsx` now imports it instead of defining its own copy; behavior is unchanged, covered by a new direct unit test.
- **T005's `InsightCard` props**: shipped as `{ claim, period, support, linkTo?, isPersonalRecord? }` rather than a numeric `sessionCount` — Consistency (weeks trained) and Push/pull balance (classified sets) don't have a literal "session count," so the shared component takes a caller-composed support string (e.g. "6 sessions", "20 classified sets", "6 of 12 weeks trained") satisfying FR-014's "how many sessions support it" in spirit for every card type without a semantic mismatch.
...
- [X] T001 Create the new directories this feature's files land in: `src/application/insights/`, `src/presentation/insights/`, `test/unit/application/insights/`, `test/unit/presentation/insights/` (no files yet — Phase 2 creates the first real content)
...
- [X] T009 [P] [US1] Unit test `test/unit/application/insights/per-exercise-progress.test.ts`: an exercise with e1RM-eligible sets across 6 distinct days spanning >=21 days in the last 90 days produces a card with the correct `ExerciseTrendResult`; an exercise with only 3 sessions, or a >=6-day span under 21 days, produces no card; a Band/FreeText-only exercise produces no card (FR-002/003, screen-contracts.md scenarios 1-4)

### Implementation for User Story 1

- [X] T010 [US1] Create `src/application/insights/per-exercise-progress.ts`: `export interface PerExerciseProgressCard { exerciseId: ExerciseId; exerciseName: string; trend: ExerciseTrendResult }` and `export function buildPerExerciseProgressCards(sessions: Session[], exercises: Exercise[]): PerExerciseProgressCard[]` — filters `sessions` to the trailing 90 days, calls `dailyE1rmValues`/`computeExerciseTrend` (T002) per Exercise, keeps only results with `distinctQualifyingDays >= 6 AND daySpanWithinWindow >= 21` (§5.7's Exercise trend row)
- [X] T011 [US1] Wire `buildPerExerciseProgressCards` into `build-insights.ts`'s `perExerciseProgress` field (replacing T004's placeholder), with `missingDataExplanation` set (e.g. "log an exercise a few more times, spanning at least 3 weeks, to see its trend") only when `cards.length === 0`
- [X] T012 [US1] Add the per-exercise-progress section to `insights-screen.tsx`: each card's claim text names the exercise and percentage change, its period is `trend.periodStart`–`trend.periodEnd`, its session count is `trend.sessionCount`, and it links to `/exercises/:exerciseId/progression` (spec 004)
...
- [X] T016 [US2] Add the recent-records section to `insights-screen.tsx`: each entry's claim names the exercise, metric, and value, its period is the 30-day window, and it links to `/exercises/:exerciseId/progression`
...
- [X] T017 [P] [US3] Unit test `test/unit/application/insights/plateau.test.ts`: an exercise with 6 distinct qualifying days in the trailing 8 weeks and an absolute percentage change under 2% produces a card; the same shape with a change of 2% or more produces no card; fewer than 6 distinct qualifying days in the 8-week window produces no card (FR-008/009, screen-contracts.md scenarios 1-3)
...
- [X] T019 [US3] Wire `buildPlateauCards` into `build-insights.ts`'s `plateau` field, with `missingDataExplanation` set only when empty AND at least one exercise has any e1RM-eligible history at all (per screen-contracts.md scenario 3's "no explanation is owed for this specific exercise" — the section-level explanation covers the card type having nothing to show overall, not every individually-non-qualifying exercise)
- [X] T020 [US3] Add the plateau section to `insights-screen.tsx`: each card's claim names the exercise and the near-zero change, its period is the trailing 8 weeks, its session count is `trend.sessionCount`, and it links to `/exercises/:exerciseId/progression`
...
- [X] T028 [US5] Add the consistency section to `insights-screen.tsx`: the card's claim states trained/covered weeks, its period is `periodStart`–`periodEnd`, no link target (this card has no single underlying exercise/session to drill into — links to the diary screen instead, per FR-9's general "links through to the raw data" intent)
