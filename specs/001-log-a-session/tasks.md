...
- [ ] T019 [P] [US1] Unit test, add to `test/unit/application/logging/use-cases.test.ts`: `createExercise(storage, input)` saves a new `Exercise` with a fresh id via `application/logging/ids.ts`'s `newExerciseId()` and returns it (FR-002, FR-015)
