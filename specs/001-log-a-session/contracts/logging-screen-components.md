...
| `LoadTypePicker` | FR-009 | One control choosing among Weight / Band / Bodyweight / Free text / None; remembers the exercise's default (FR-009), overridable per set. |
| `WeightLoadInput` | FR-010, FR-026 | Numeric keypad by default; quick-increment buttons (research.md §9) that clamp at 0, never go negative (FR-010) — clamping is itself a visible state (the down-increment button shows disabled-with-reason at 0, not a silent no-op). |
