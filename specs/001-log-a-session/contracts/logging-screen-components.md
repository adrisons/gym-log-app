...
| `LogWorkoutControl` | FR-027 (ADR-0008, ADR-0012) | Always rendered, `disabled` (never removed) while the active draft has no exercise (`draftHasContent`) or a pending-draft banner is unresolved — the same disabled-not-hidden convention `SetConfirmControl`, below, already uses (ADR-0012 supersedes this row's earlier "visible only while..." text). Converts the draft to a Session, resets the screen to a fresh, empty draft, and navigates to the diary on success; this is what the `SessionSavedToast`/`justRegisteredWorkout` acknowledgement is tied to now, not `addSet`. |
| `BlockList` / `BlockCard` | FR-006, FR-007, FR-004, FR-023, FR-2 (`docs/requirements.md`), ADR-0013 | Unnamed block renders its position label (FR-007) — computed by `view-models.ts`, never hardcoded per-component. Delete action shows the 5-second undo (shared `UndoToast`, below), offered through the same secondary-menu affordance as `ExerciseEntryCard`'s and each set's delete action (`docs/design.md` §2). Collapse/expand is local UI state, not persisted, and never affects what the block's own undo restores. Move up/Move down (ADR-0013, this menu only — no inline copy) reorders the block among its session's siblings, mirroring `ExerciseEntryCard`'s own Move up/Move down one level up. No round-count field of any kind (ADR-0008 added one, ADR-0013 removed it again as redundant with each exercise entry's own set count). |
| `ExerciseEntryCard` | FR-002, FR-004 | Reorder handles need a keyboard-operable equivalent (`docs/design.md` §5 — "nothing essential revealed by hover/pointer only"), not just drag. Move up/Move down/Move-to-block are wired identically on both screens that render this component (`LoggingScreen` and `SessionDetailScreen`) — a gap where `SessionDetailScreen` passed no-op handlers here was closed by ADR-0014, not a difference the contract ever intended. |
...
| `VolumeInput` (reps / duration / distance) | FR-008, FR-026 | Reps: a scrollable 1–100 wheel (plus an unset position) in `SetRow`'s stacked form. _(Amended by ADR-0015: for the common Weight+Reps, no-effort case, `SetRow`'s compact single-line layout renders reps as a plain integer keypad field sharing the sets table's own columns instead — every other load/volume/effort combination keeps the wheel unchanged.)_ Duration/distance: numeric keypad, unchanged. Volume must be > 0 — same clamp-and-disabled-reason pattern as load. |
...
| `SetConfirmControl` | FR-003, FR-019, FR-025 (ADR-0010) | **Retired by ADR-0007, then restored by ADR-0010** — an explicit "Add set"/"Save changes" button again, `disabled` (silently — ADR-0014 removed the status line that used to name exactly what was missing) until the row holds every field the exercise's current template tracks. No per-field auto-commit and no debounce on entry (a wheel step, a quick-increment tap, or fast typing no longer schedules anything on its own — the button is the only trigger); FR-025's short identical-within-1s dedupe still guards the store-level commit itself, for a double-tap. Also drives editing an already-recorded set in place (new capability): same control, labelled "Save changes", gated by the domain minimum (a load or a volume) rather than the fuller per-field rule, since editing starts from an already-valid set. _(ADR-0015: in the compact single-line layout, this renders as a small trailing "+"/checkmark icon button sharing the row's grid, not the full-width labelled button the stacked form uses — same `disabled` gating, no visible-state change.)_ |
| `SetSummary` (each already-logged set's row) | FR-003, FR-019 (ADR-0013, ADR-0015) | Renders as a row of a Reps/Duration/Distance + Load table (`docs/design.md` §1.2's ADR-0015 refinement note) — a header row above the list names each column, sourced from the exercise's own `volumeKind`, never hard-coded. _(Amended by ADR-0015: the previous "⋮" `OverflowMenu` offering Edit/Delete is replaced by a bare trailing "×" that only deletes; tapping the row's own value cells opens the same in-place edit form Edit used to, so editing stays reachable with one fewer control per row.)_ An absent column (a `none`-kind load, or a load-only set with no volume) renders empty — never a "—" placeholder (ADR-0012, unchanged). |
| ~~`RepeatLastSetControl`~~ | FR-008, FR-025 | **Retired by ADR-0010.** Existed only because ADR-0007 had no general confirm control to fall back on for a pre-filled-and-untouched row; now that `SetConfirmControl` is back, a pre-filled row is simply already enabled, so confirming an identical repeat (FR-008) is still one tap, on the same control every other set uses. |
...
- Hit targets sized for one-handed, sweaty-hands use, especially
  `SetConfirmControl` and `UndoToast`'s undo action (the quick-increment
