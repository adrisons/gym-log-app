...
| `LogWorkoutControl` | FR-027 (ADR-0009) | A single button, visible only while the active draft has at least one block (`draftHasContent`) — unavailable rather than a disabled-with-reason state (contrast `SetConfirmControl`, above, which stays visible and disabled-with-reason instead, since ADR-0010). Converts the draft to a Session, resets the screen to a fresh, empty draft, and navigates to the diary on success; this is what the `SessionSavedToast`/`justRegisteredWorkout` acknowledgement is tied to now, not `addSet`. |
...
| `SetConfirmControl` | FR-003, FR-019, FR-025 (ADR-0010) | **Retired by ADR-0007, then restored by ADR-0010** — an explicit "Add set"/"Save changes" button again, `disabled` with a status line naming exactly what the exercise's current template still needs (FR-019) until the row holds every field it tracks. No per-field auto-commit and no debounce on entry (a wheel step, a quick-increment tap, or fast typing no longer schedules anything on its own — the button is the only trigger); FR-025's short identical-within-1s dedupe still guards the store-level commit itself, for a double-tap. Also drives editing an already-recorded set in place (new capability): same control, labelled "Save changes", gated by the domain minimum (a load or a volume) rather than the fuller per-field rule, since editing starts from an already-valid set. |
