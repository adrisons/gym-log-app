 ...
 * methods by "Sessions" / "Exercise catalogue" / "The logging draft" /
 * schema version) — the simplest possible mapping, nothing to
...
/** Single-row tables (`StoragePort`'s draft/settings/schema-version surface), keyed by a fixed string so there is always exactly zero or one row. */
